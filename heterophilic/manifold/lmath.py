...
def norm(u, *, keepdim=False, dim=-1):
    ...
    .. math::
        ...
        \|\mathbfv}\|_{\mathcal{L}}=\sqrt{\langle\mathbf{v}, \mathbf{v}\rangle_{\mathcal{L}}}
...
def _expmap(x, u, k: torch.Tensor, dim: int = -1):
    ...
    nomin = (_norm(u, keepdim=True, dim=dim)) * k.sqrt()
    u = u / nomin
...
def _logmap0(y, k, dim: int = -1):
    ...
    zero_point[0] = 1
    return acosh(alpha) / torch.sqrt((alpha * alpha - 1).clamp_min(1e-6)) * (y - alpha * zero_point)
