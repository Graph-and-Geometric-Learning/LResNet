from models import *
from hyp_transformer import *
from lrn import *
from manifold.lorentz import *
...
def parse_method(method, args, c, d, device):
    if method == 'gcn':
    ...
    elif args.method == 'hyp_transformer':
        curvature = torch.tensor([1.0]).to(device)
        gnn=parse_method(args.backbone, args, args.hidden_channels, d, device)
        model = HypSGFormer(curvature, d, args.hidden_channels, c, args.n, num_layers=args.ours_layers, alpha=args.alpha, dropout=args.ours_dropout, num_heads=args.num_heads,
        ...
        acts += [act]
        dims = [d + 1] + ([args.hidden_channels + 1] * (args.num_layers - 1))
        dims += [c + 1]
        model = HyboNet(curvature, manifold, acts, dims, args)
    elif method == 'Skip':
        if args.cpu:
            device = torch.device("cpu")
        else:
        ...
        dims = [d + 1] + ([args.hidden_channels + 1] * (args.num_layers - 1))
        dims += [c + 1]
        model = SkipHGNN(curvature, manifold, acts, dims, args)
    else:
