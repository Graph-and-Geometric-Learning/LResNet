...
def train(args):
    ...
    #     print("Using seed {}.".format(args.seed), file = f)
    with open("output.txt", "a") as f:
        print("Using dataset: {}.".format(args.dataset), file = f)
    for epoch in range(args.epochs):
        t = time.time()
        model.train()
        optimizer.zero_grad()
        embeddings = model.encode(data['features'], data['adj_train_norm'])
        # U, S, Vt = np.linalg.svd(embeddings.detach().numpy())
        # threshold = 1e-10
    ...
    with open("output.txt", "a") as f:
        print(" ".join(
        ["Test set results:",
         format_metrics(best_test_metrics, 'test')]), file=f)
    if args.save:
        np.save(os.path.join(save_dir, 'embeddings.npy'),
